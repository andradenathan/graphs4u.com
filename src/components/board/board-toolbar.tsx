...
export function BoardToolbar() {
    ...
    return (
        <>
            {/* Desktop toolbar - left floating */}
            <div
                data-slot="board-toolbar"
                className="absolute left-3 top-3 z-10 hidden md:flex flex-col gap-1 rounded-xl border border-border bg-surface/90 p-1.5 shadow-lg backdrop-blur-sm"
            >
                {tools.map((tool) => {
            ...
            <div
                data-slot="board-toolbar-mobile"
                className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 md:hidden flex gap-1 rounded-xl border border-border bg-surface/90 p-1.5 shadow-lg backdrop-blur-sm"
            >
                {tools.map((tool) => {
                    const Icon = tool.icon;
                    const label = t(tool.i18nKey);
